...
import { CheckCircle2, XCircle, ArrowLeft, RefreshCw, Wallet } from 'lucide-react';
...
export const MomoCallbackPage = () => {
  ...
  useEffect(() => {
    if (isSuccess) {
      toast.success('🎉 Nạp tiền MoMo thành công! Số dư ví của bạn đã được cập nhật.');
    } else {
      ...
      <div className="max-w-md w-full mx-auto px-4 text-center space-y-6">
        ...
        {isSuccess ? (
          <div className="bg-white p-8 rounded-3xl border border-emerald-200 shadow-xl space-y-4 animate-fadeIn">
            ...
            <p className="text-xs text-slate-500 font-medium">Cổng MoMo Sandbox đã ghi nhận giao dịch của bạn.</p>

            <div className="bg-slate-50 p-4 rounded-2xl border border-slate-100 space-y-2 text-left text-xs">
              <div className="flex justify-between">
                <span className="text-slate-500 font-medium">Mã giao dịch MoMo:</span>
                <span className="font-mono font-bold text-slate-900 truncate max-w-[180px]">{orderId || 'MOMO_TOPUP_SUCCESS'}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-slate-500 font-medium">Số tiền nạp:</span>
                <span className="font-mono font-black text-emerald-600 text-sm">+{Number(amount).toLocaleString('vi-VN')} VNĐ</span>
              </div>
              <div className="flex justify-between">
                <span className="text-slate-500 font-medium">Trạng thái MoMo:</span>
                <span className="font-bold text-emerald-600">THÀNH CÔNG (code 0)</span>
              </div>
            </div>
          ...
          <div className="bg-white p-8 rounded-3xl border border-rose-200 shadow-xl space-y-4 animate-fadeIn">
            ...
            <h2 className="text-xl font-black text-slate-900">Giao Dịch Thất Bại Hoặc Bị Hủy</h2>
